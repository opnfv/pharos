...
##############################################################################
"""Library for jinja2 template operations, including all custom filters"""
...
def load_custom_filters(environment):
    """Load all defined filters into the jinja2 library"""
...
def filter_ipnet_hostaddr(network_address, prefixlen, index):
    ...
    try:
        ...
        logging.error(ipnet.with_prefixlen + " has not enough range for " + str(index)
            + " host IPs. Out of range after " + str(ipnet.num_addresses))
        raise
