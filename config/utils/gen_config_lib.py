...
def filter_ipnet_hostaddr(network_address, prefixlen, index):
    ...
    try:
        network_addr_str = unicode(network_address)
        prefixlen_str = unicode(prefixlen)
    except NameError as ex:
        network_addr_str = str(network_address)
        prefixlen_str = str(prefixlen)

    try:
        ipnet = ipaddress.IPv4Network(network_addr_str + "/" + prefixlen_str)
        return list(ipnet.hosts())[index - 1]
    except ValueError as ex:
        logging.error(network_addr_str + "/" + prefixlen_str +
